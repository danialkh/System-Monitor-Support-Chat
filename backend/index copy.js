...
async function triggerGeminiBotReply(userPrompt) {
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      contents: userPrompt,
      config: {
        systemInstruction: "You are a helpful real-time support assistant embedded in a live dashboard. Keep your answers concise, conversational, and helpful.",
        maxOutputTokens: 150,
      }
    });

    // Console logs to inspect the raw response and text in terminal
    console.log("Full Gemini Response:", JSON.stringify(response, null, 2));
    
    // Extract response text safely using SDK structure
